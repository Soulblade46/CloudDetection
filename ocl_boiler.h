	...
	cl_device_id dev)
{
	...
	// add '-I.' to compile options 
	snprintf(all_compile_opt, BUFSIZE, "%s %s",
		"-I.", compile_opt ? compile_opt : "");

	printf("Building '%s' with options '%s'\n",
		fname, all_compile_opt);
	err = clBuildProgram(prg, 1, &dev, all_compile_opt, NULL, NULL);
...
// Time elapsed between the start of evt1 and the end of evt2
cl_ulong elapsed_ns(cl_event evt1, cl_event evt2)
{
	cl_int err;
	cl_ulong start, end;
	err = clGetEventProfilingInfo(evt1, CL_PROFILING_COMMAND_START,
		sizeof(start), &start, NULL);
	ocl_check(err, "get start");
	err = clGetEventProfilingInfo(evt2, CL_PROFILING_COMMAND_END,
		sizeof(end), &end, NULL);
	ocl_check(err, "get end");
	return (end - start);
}
